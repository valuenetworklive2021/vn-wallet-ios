//
//  PredictionMarketsCoordinator.swift
//  AlphaWallet
//
//  Created by Aleksandr Ivannikov on 08.11.2021.
//
...
protocol PredictionMarketsCoordinatorDelegate: class, CanOpenURL {
...
final class PredictionMarketsCoordinator: NSObject, Coordinator {
    ...
    lazy var rootViewController: DappsHome2ViewController = {
        let vc = DappsHome2ViewController(bookmarksStore: bookmarksStore)
        vc.delegate = self
        return vc
    }()

    weak var delegate: PredictionMarketsCoordinatorDelegate?
   ...
   init(
    ...
    func start() {
        ...
        if let url = config.predictionMarketsAppURL {
            open(url: url, animated: true, forceReload: true)
        } else {
        }
    }

    func showPredictionMarketsHome() { // showDappsHome
    ...
    private func createPredictionMarketsViewController() -> MyDappsViewController {
    ...
    func open(url: URL, animated: Bool = true, forceReload: Bool = false) {
        ...
        navigationController.removeViewControllerOfSameType2(except: browserViewController)
    ...
    private func makeMoreAlertSheet(sender: UIView) -> UIAlertController {
        ...
        let setAsHomePageAction = UIAlertAction(title: R.string.localizable.setAsHomePage(), style: .default) { [weak self] _ in
            self?.config.predictionMarketsPageURL = self?.currentUrl
...
extension PredictionMarketsCoordinator: TransactionConfirmationCoordinatorDelegate {
...
extension PredictionMarketsCoordinator: BrowserViewControllerDelegate {
...
extension PredictionMarketsCoordinator: BrowserHistoryViewControllerDelegate {
...
extension PredictionMarketsCoordinator: WKUIDelegate {
...
extension PredictionMarketsCoordinator: DappsHomeViewControllerDelegate {

    private func showPredictionMarketsDapps() {
        //logShowDapps()
        //let viewController = createMyDappsViewController()
        //pushOntoNavigationController(viewController: viewController, animated: true)
        //navigationController.removeViewControllerOfSameType2(except: viewController)

        guard let url = URL(string: "https://app.valuenetwork.live/") else { return }
        open(url: url, animated: false)
    }
    ...
    func didTapPredictionMarketsViewController(inViewController viewController: DappsHomeViewController) {
        showPredictionMarketsDapps()
    }
...
extension PredictionMarketsCoordinator: DappsHome2ViewControllerDelegate {

    private func showPredictionMarketsDapps2() {
        //logShowDapps()
        //let viewController = createMyDappsViewController()
        //pushOntoNavigationController(viewController: viewController, animated: true)
        //navigationController.removeViewControllerOfSameType2(except: viewController)
    ...
    private func showBrowserHistory2() {
        logShowHistory()
        let viewController = createHistoryViewController()
        pushOntoNavigationController(viewController: viewController, animated: true)
        navigationController.removeViewControllerOfSameType2(except: viewController)
...
extension PredictionMarketsCoordinator: DiscoverDappsViewControllerDelegate {
...
extension PredictionMarketsCoordinator: MyDappsViewControllerDelegate {
...
extension PredictionMarketsCoordinator: DappsAutoCompletionViewControllerDelegate {
...
extension PredictionMarketsCoordinator: DappBrowserNavigationBarDelegate {
...
extension PredictionMarketsCoordinator: EditMyDappViewControllerDelegate {
...
extension PredictionMarketsCoordinator: ScanQRCodeCoordinatorDelegate {
...
extension PredictionMarketsCoordinator: ServersCoordinatorDelegate {
...
extension PredictionMarketsCoordinator: CanOpenURL {
...
// MARK: Analytics
extension PredictionMarketsCoordinator {
...
extension PredictionMarketsCoordinator: DappRequestSwitchCustomChainCoordinatorDelegate {
...
extension PredictionMarketsCoordinator: DappRequestSwitchExistingChainCoordinatorDelegate {
...
extension UINavigationController {
    ...
    func removeViewControllerOfSameType2<T>(except avoidToRemove: T) where T: UIViewController {
